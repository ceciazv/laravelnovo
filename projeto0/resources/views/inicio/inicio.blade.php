...
<body>
  <header>
    ...
    <div class="hidden fixed top-0 right-0 px-6 py-4 sm:block">
      @auth
      <a href="{{ url('/dono') }}" class="text-sm text-gray-700 underline">Área Administrativa</a>
      @else
  ...
  <main>
    ...
    <aside>
      @foreach($initial->donos() as $donos)
      <div><img src="{{$donos->user->profile_photo_url}}"> {{$donos->user->name}}</div>
      @endforeach
    </aside>
  </main>
</body>
